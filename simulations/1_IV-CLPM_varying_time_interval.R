## CLPM + IVR with Varying Time Intervals

## Aim: Effect of Time Interval on Bidirectional Proximal and Distal Effects
...
getwd()
resfile <- 'resfile_varyingLag_bidir_IV-CLPM.dta'  # output file for the results
        ...
        for (r_xy in rxys) {         # Residual Correlation
          for (idis in Tdis) {       # Panel Lag
            ...
            if(bxy==max(bxys) & byx==max(byxs) & bx==max(bxs) & by==max(bys) & r_xy==min(rxys)) {
              ...
              # covariances at the farthest wave2
              if (idis == max(Tdis)) {
...
## max R2
table(round(100 * keep[which(keep$bxx==.7 & keep$byy==.7 & keep$bxy==0.2 & keep$byx==0.2 & keep$rxy==0.1),]$`IVx->X1`,2))

table(round(100 * keep[which(keep$bxx==.7 & keep$byy==.7 & keep$bxy==0.2 & keep$byx==0.2 & keep$rxy==0.1),]$`IVx->X2`,2))
# 8.03%


## min R2

table(round(100 * keep[which(keep$bxx==.5 & keep$byy==.5 & keep$bxy==0.1 & keep$byx==0.1 & keep$rxy==0.3),]$`IVx->X1`,2))

table(round(100 * keep[which(keep$bxx==.5 & keep$byy==.5 & keep$bxy==0.1 & keep$byx==0.1 & keep$rxy==0.3),]$`IVx->X2`,2))
...
table(round(keep[which(keep$bxx==.5 & keep$byy==.5 & keep$bxy==0.1 & keep$byx==0.1 & keep$rxy==0.1),]$std_prox1_xy, 3))

summary(keep[which(keep$bxx==.5 & keep$byy==.5 & keep$bxy==0.1 & keep$byx==0.1 & keep$rxy==0.1),]$std_distal_xy)

table(round(keep[which(keep$bxx==.5 & keep$byy==.5 & keep$bxy==0.1 & keep$byx==0.1 & keep$rxy==0.1),]$std_prox2_xy, 3))
...
keep |> 
  filter(bxx==.7, byy==.7, bxy==0.2, byx==0.2, rxy==0.1) |> 
  pivot_longer(cols = c('NCP_all_bd','NCP_all_xy','NCP_dist_prox2_xy'), 
  ...
  geom_hline(yintercept = qchisq(0.05, df = 2, ncp = 0, lower.tail = F),
                                                     linetype = "dashed", color = "grey75") +
  geom_hline(yintercept = qchisq(0.05, df = 3, ncp = 0, lower.tail = F),
             linetype = "dashed", color = "grey75") +
  geom_hline(yintercept = qchisq(0.05, df = 6, ncp = 0, lower.tail = F),
             linetype = "dashed", color = "grey75") +
  scale_color_brewer(palette = "Dark2") +
  # coord_cartesian(xlim = c(0,40)) + 
  labs(title = "LRT Statistic as a Function of Study Time Interval",
       y = "LRT Statistic", x = "Study Panel Lag", color = "Parameter") +
  theme_bw(12) + 
  theme(legend.position = "bottom")



## 1-df tests for the three estimates in either direction

keep |> 
  filter(bxx==.7, byy==.7, bxy==0.2, byx==0.2, rxy==0.1) |> 
  pivot_longer(cols = c("NCP_prox_xy1","NCP_distal_xy","NCP_prox_xy2"), 
               names_to = "parameter", names_prefix = "NCP_",
               values_to = "ncp") %>% 
  mutate(bxy = as_factor(bxy),
         parameter = as_factor(parameter)
...
## Previous version Split into 2 panels
...
keep %>% 
  ...
  mutate(parameter = as_factor(parameter),
         parameter = fct_relevel(parameter, "prox1_bd"),
         lrt = ifelse(parameter %in% c("prox1_bd","distal_bd","prox2_bd"), "2-DF",
                      "4-/6-DF")) %>% 
  ggplot(aes(x = Tdis, y = ncp, color = parameter)) +
  geom_line(stat = "summary") +
  geom_point(size=1.5, stat = "summary") +
  facet_wrap(~lrt) +
  scale_color_brewer(palette = "Dark2") + 
  coord_cartesian(xlim = c(0,40)) +
  labs(title = "Bidirectional LRTs in IV-CLPM",
